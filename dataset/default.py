...
class DEFAULTDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        drr1 = torch.tensor(np.array(Image.open(self.file_paths[idx].replace('dataSets', 'dataSetsDRR-128').replace('.nii.gz', '_1.png')))[np.newaxis, :, :],dtype=torch.float32)
        drr2 = torch.tensor(np.array(Image.open(self.file_paths[idx].replace('dataSets', 'dataSetsDRR-128').replace('.nii.gz', '_2.png')))[np.newaxis, :, :],dtype=torch.float32)
        ...
        return {'data': img.data.permute(0, -1, 1, 2), 'drr1': drr1, 'drr2': drr2,'affine': affine,'spacing': spacing}
